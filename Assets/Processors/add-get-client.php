...
try {
    ...
    if ($method === 'POST') {
        ...
        echo json_encode(['success' => true, 'message' => 'Client added successfully!']);
            ...
            $query = "SELECT CID, Name, PhoneNumber, Email, ProjectName, ProjectDescription, Budget, OpeningBalance, ClosingBalance, ClientStatus FROM clients";
            if ($status === 'active') {
                $query .= " WHERE ClientStatus = 'Active'";
            } elseif ($status === 'inactive') {
                $query .= " WHERE ClientStatus = 'Inactive'";
            }

            $clients = $db->query($query)->fetchAll(PDO::FETCH_ASSOC);
